...
def _parse_list_module_file(
    path: Path, spec: DomainSpec, warnings: list[str]
    ...
    if error:
        ...
        return [], [], False, False
    rel_path = path.relative_to(settings.CONFIG_DIR).as_posix()
    items, changed = _parse_list_items(data, lines, rel_path, spec, warnings)
...
def _sync_list_domain(
    ...
    for path in module_files:
        items, _data, injected, valid = _parse_list_module_file(path, spec, warnings)
        rel_path = path.relative_to(settings.CONFIG_DIR).as_posix()
        if not valid:
    ...
    domain_items, _domain_data, domain_injected, domain_valid = _parse_list_domain(spec, warnings)
...
def _sync_mapping_domain(
    ...
    for path in module_files:
        items, _data, _injected, valid = _parse_mapping_module_file(path, warnings)
        rel_path = path.relative_to(settings.CONFIG_DIR).as_posix()
        if not valid:
            invalid_module_files.add(rel_path)
            continue
