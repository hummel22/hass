...
def _integration_sort_key(entry: IntegrationEntry) -> tuple[str, str]:
    """Return a key used to sort integrations alphabetically."""

    for value in (entry.title, entry.domain, entry.entry_id):
        if value:
            return (value.casefold(), value)
    return ("", entry.entry_id)


async def _load_available_integrations() -> List[IntegrationEntry]:
    """Fetch integrations that have at least one associated device."""

    try:
        integrations, device_registry = await asyncio.gather(
            hass_client.fetch_integrations(),
    ...
    for device in device_registry:
        ...
        for entry_id in config_entries:
            if isinstance(entry_id, str) and entry_id:
                device_entry_ids.add(entry_id)

    available: List[IntegrationEntry] = []
    ...
    available.sort(key=_integration_sort_key)
    return available
...
async def available_integrations() -> List[IntegrationEntry]:
    ...
    return await _load_available_integrations()
...
async def selected_integrations() -> List[IntegrationEntry]:
    selected = [IntegrationEntry(**entry) for entry in repository.get_selected_integrations()]
    selected.sort(key=_integration_sort_key)
    return selected
...
async def add_integration(request: IntegrationSelectionRequest) -> List[IntegrationEntry]:
    ensure_hass_configured()
    try:
        available = await _load_available_integrations()
    except HTTPException:
        raise

    integration_map = {entry.entry_id: entry for entry in available}
    target = integration_map.get(request.integration_id)
    if not target:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Integration not found or has no associated devices",
        )

    repository.add_integration(target.dict())
    selected = [IntegrationEntry(**entry) for entry in repository.get_selected_integrations()]
    selected.sort(key=_integration_sort_key)
    return selected
